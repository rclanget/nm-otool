...
static struct ranlib_64	*sort_ranlib(struct ranlib_64 *rlib, uint64_t nranlib)
{
	...
	bzero(rlib_sort, sizeof(struct ranlib_64) * (nranlib));
	memcpy(rlib_sort, rlib, sizeof(struct ranlib_64) * nranlib);
	is_sorted = 0;
	i = 0;
	while (!is_sorted)
	{
		...
		while ((i + 1) < nranlib)
		{
			...
			{
				memcpy(&buf, &rlib_sort[i], sizeof(struct ranlib_64));
				memcpy(&rlib_sort[i], &rlib_sort[i + 1], sizeof(struct ranlib_64));
				memcpy(&rlib_sort[i + 1], &buf, sizeof(struct ranlib_64));
				is_sorted = 0;
			}
			i++;
		}
